...
declare const STAGE_OPERATOR_NAMES: ("$count" | "$addFields" | "$bucket" | "$bucketAuto" | "$changeStream" | "$collStats" | "$currentOp" | "$densify" | "$documents" | "$facet" | "$fill" | "$geoNear" | "$graphLookup" | "$group" | "$indexStats" | "$limit" | "$listLocalSessions" | "$lookup" | "$match" | "$merge" | "$out" | "$project" | "$rankFusion" | "$redact" | "$replaceRoot" | "$replaceWith" | "$sample" | "$scoreFusion" | "$search" | "$searchMeta" | "$set" | "$setWindowFields" | "$skip" | "$sort" | "$sortByCount" | "$unionWith" | "$unset" | "$unwind" | "$vectorSearch")[];
