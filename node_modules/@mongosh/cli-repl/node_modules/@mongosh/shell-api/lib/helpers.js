...
exports.aggregateBackgroundOptionNotSupportedHelp = exports.onlyShardedCollectionsInConfigFilter = void 0;
...
let coreUtilInspect;
function adaptAggregateOptions(options = {}, hasUnifiedAggregateOptions = true) {
...
function assertArgsDefinedType(args, expectedTypes, func) {
    args.forEach((arg, i) => {
        ...
        const isExpectedBson = expectedTypesList.includes(`bson:${(0, shell_bson_1.getBsonType)(arg)}`);
...
async function computeLegacyHexMD5(sp, str) {
    ...
    try {
        ...
        throw new errors_1.MongoshUnimplementedError('Legacy password digest algorithms like SCRAM-SHA-1 are not supported by this instance of mongosh', errors_1.CommonErrors.Deprecated);
    }
    const hash = crypto.createHash('md5');
    hash.update(str);
    return hash.digest('hex');
}
async function processDigestPassword(username, passwordDigestor, command, sp) {
                    ...
                    if (coreUtilInspect) {
                        Object.defineProperty(c, Symbol.for('nodejs.util.inspect.custom'), {
                            value: function (depth, options) {
                                return coreUtilInspect({ ...this }, { ...options, breakLength: Infinity });
                            },
                            writable: true,
                            configurable: true,
                        });
...
function processFLEOptions(fleOptions) {
    ...
    if (localKey && (0, shell_bson_1.getBsonType)(localKey) === 'Binary') {
...
function isBSONDoubleConvertible(val) {
    return ((typeof val === 'number' && Number.isInteger(val)) ||
        (0, shell_bson_1.getBsonType)(val) === 'Int32');
}
