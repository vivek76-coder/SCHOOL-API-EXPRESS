...
let ShellConfig = (() => {
    ...
    var ShellConfig = _classThis = class extends _classSuper {
        constructor(instanceState) {
            super();
            this._instanceState = __runInitializers(this, _instanceExtraInitializers);
...
async function newUseCompleter(context, args) {
    if (args.length > 2)
        return undefined;
    ...
    return dbNames.filter((name) => name.toLowerCase().startsWith(prefix) && !CONTROL_CHAR_REGEXP.test(name));
}
function completeShowCommand(prefix) {
    if (prefix === 'd') {
        return ['databases'];
    }
    const candidates = [
...
async function newShowCompleter(context, args) {
    if (args.length > 2)
        return undefined;
    return completeShowCommand(args[1] ?? '');
}
let ShellApi = (() => {
    ...
    var ShellApi = _classThis = class extends _classSuper {
        constructor(instanceState) {
            super();
            this[_a] = __runInitializers(this, _instanceExtraInitializers);
    ...
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
        _use_decorators = [decorators_1.directShellCommand, (0, decorators_1.shellCommandCompleter)(useCompleter, newUseCompleter)];
        _show_decorators = [decorators_1.directShellCommand, decorators_1.returnsPromise, (0, decorators_1.shellCommandCompleter)(showCompleter, newShowCompleter)];
