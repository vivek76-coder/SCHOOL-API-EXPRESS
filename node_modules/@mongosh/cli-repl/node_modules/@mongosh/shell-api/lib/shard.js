...
let Shard = (() => {
    ...
    let _enableMigrations_decorators;
    let _disableMigrations_decorators;
    ...
    var Shard = _classThis = class extends _classSuper {
        constructor(database) {
            super();
            this._database = __runInitializers(this, _instanceExtraInitializers);
        ...
        [enums_1.asPrintable]() {
            return `Shard class connected to ${(0, mongodb_redact_1.redactConnectionString)(this._database._mongo._uri)} via db ${this._database._name}`;
        }
        ...
        async status(verbose = false, configDB) {
            const result = await (0, helpers_1.getPrintableShardStatus)(configDB ?? (await (0, helpers_1.getConfigDB)(this._database)), verbose);
        ...
        async enableAutoSplit() {
            ...
            if (connectionInfo?.buildInfo?.version &&
        ...
        async _setAllowMigrations(ns, allowMigrations) {
            const apiCall = `${allowMigrations ? 'enable' : 'disable'}Migrations`;
            (0, helpers_1.assertArgsDefinedType)([ns], ['string'], `Shard.${apiCall}`);
            this._emitShardApiCall(apiCall, { ns });
            const helloResult = await this._database._maybeCachedHello();
            if (helloResult.msg !== 'isdbgrid') {
                await this._database._instanceState.printWarning('MongoshWarning: [SHAPI-10003] You are not connected to a mongos. This command may not work as expected.');
            }
            return await this._database._runAdminCommand({
        ...
        async enableMigrations(ns) {
            return await this._setAllowMigrations(ns, true);
        }
        async disableMigrations(ns) {
    ...
    (() => {
        ...
        _enableMigrations_decorators = [decorators_1.returnsPromise, (0, decorators_1.apiVersions)([])];
        _disableMigrations_decorators = [decorators_1.returnsPromise, (0, decorators_1.apiVersions)([])];
        ...
        __esDecorate(_classThis, null, _enableMigrations_decorators, { kind: "method", name: "enableMigrations", static: false, private: false, access: { has: obj => "enableMigrations" in obj, get: obj => obj.enableMigrations }, metadata: _metadata }, null, _instanceExtraInitializers);
        __esDecorate(_classThis, null, _disableMigrations_decorators, { kind: "method", name: "disableMigrations", static: false, private: false, access: { has: obj => "disableMigrations" in obj, get: obj => obj.disableMigrations }, metadata: _metadata }, null, _instanceExtraInitializers);
